...
If you open the application you will get four main pages: [Map](#map), [Assets](#assets), [Rules](#rules), and [Insights](#insights). In addition there are a series of `Settings` to interconnect managers, change language, edit your account, and create users, roles, or realms. Also the service users for the `Manager APIs` can be set here. We'll discuss all views as well as the additional settings options.
