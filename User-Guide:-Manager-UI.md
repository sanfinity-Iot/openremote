...
_Figure 2. Creating an asset of the type 'environment' (top) and the resulting Asset page (bottom)_
...
_Figure 3. The Asset view in 'Edit mode' (left) and adding an attribute (right)_
...
_Figure 4. Creating an Agent, using the HTTP Agent (left) creates the Agent asset page (right)_
...
_Figure 5. A WHEN-THEN Rules example, which shows how, on the lefthand-side an asset attribute condition can be selected, while on the righthand-side the action is selected._
...
_Figure 6. The Rules trigger frequency (left) as well as time scheduler (right) allows for defining when rules are triggered and active_
...
_Figure 7. Flow rules to process data (left) and Groovy rules for programming more advanced logic (right)_
...
_Figure 8. The Insights view of the Demo Smart City, showing the soil temperature at Leuven Haven and a few random attribute panels._
...
_Figure 10. The Logs page to evaluate system behaviour._
...
_Figure 11. The account page which allows setting e.g. contact details (left) and reset passwords (right)._

![](https://github.com/openremote/Documentation/blob/master/manuscript/figures/Manager%20-%20Users%20(2).png)
_Figure 12. Creating users for a selected Realm and assigning roles (left). Also service users are created here (right)_

![](https://github.com/openremote/Documentation/blob/master/manuscript/figures/Manager%20-%20Roles.png)
_Figure 13. Roles can be defined which can be linked to individual users_

### Realms

![](https://github.com/openremote/Documentation/blob/master/manuscript/figures/Manager%20-%20Realms.png)
_Figure 14. Realms can be created to manage multiple independent projects within one OpenRemote instance_
