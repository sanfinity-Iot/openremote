TODO:
- describe correct basic auth on controller
- what example sensors and commands to use from the Home Example
...
# Declaring Home Example Controller Agent
In the following example, you will link to the Home Example Demo Controller 2.5 on `http://demo.openremote.com:8688/controller`
...
In the following example, we assume we already have a running Controller 2.5 on `http://my.controller:8688/controller`
...
You now have a Controller protocol to communicate with one running Controller 2.5 as soon as it will be available on network and attribute will be linked.

The attribute status is DISCONNECTED until a Controller 2.5 is really available on the provided base URL.

We can now add linked attribute in Manager to the new Agent such that we can get sensors status and execute commands

# Configuring Attributes to use a Controller Agent
For each different case, we will consider concrete example and how to implement them in the Manager UI.
